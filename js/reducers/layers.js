...
import LayersUtils from '../../MapStore2/web/client/utils/LayersUtils';
...
export function layers(state = { flat: [] }, action) {
    switch (action.type) {
    case TOGGLE_CONTROL: {
    ...
    case SET_DEFAULT_EXPANDED: {
        let newLayers = state?.groups.map((layer) => {
            const layerGroup = layer.group || 'Default';
            if (layerGroup === action.group || layerGroup.indexOf(`${action.group}.`) === 0) {
                return assign({}, layer, action.expanded);
            }
            return assign({}, layer);
        });
        return assign({}, state, {
